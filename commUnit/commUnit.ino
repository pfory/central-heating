...
//Adafruit_MQTT_Subscribe restart                = Adafruit_MQTT_Subscribe(&mqtt, MQTTBASE "restart");
  SenderClass sender;
  sender.add("tINKamna", tempINKamna);
  sender.add("tOUTKamna", tempOUTKamna);
  sender.add("sPumpKamna/status", pumpStatus);
  sender.add("t0", temp[0]);
  sender.add("t1", temp[1]);
  sender.add("t2", temp[2]);
  sender.add("t3", temp[3]);
  sender.add("t4", temp[4]);
  sender.add("t5", temp[5]);
  sender.add("t6", temp[6]);
  sender.add("t7", temp[7]);
  sender.add("t8", temp[8]);
  sender.add("t9", temp[9]);
  sender.add("t10", temp[10]);
  sender.add("t11", temp[11]);
